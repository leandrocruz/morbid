...
object client {
  ...
  trait MorbidClient {
    ...
    def impersonate       (request: ImpersonationRequest)      (using token: RawToken                      ): Task[String]
  }
  ...
  case class RemoteMorbidClient(base: URL, client: Client, scope: Scope) extends MorbidClient {
    ...
    override def impersonate       (request: ImpersonationRequest)      (using token: RawToken                      ) = post[ImpersonationRequest, String]                        (Some(token),  base / "impersonate", request)
  }
